...
<body>
    ...
    <div class="w3-top">
        <div class="w3-bar w3-card w3-container w3-text-white w3-mobile">
            <a href="{{route('user.index')}}" class="w3-mobile w3-bar-item w3-button w3-padding-large w3-hide-small w3-left w3-hover-indigo">TeachEdison Solutions Private Limited</a>

            <a href="{{route('user.create')}}" class=" w3-mobile w3-bar-item w3-button w3-padding-large w3-hide-small w3-right w3-hover-indigo">Register</a>
        </div>
    </div>
    <div class="w3-display-middle w3-padding-large w3-card-4 w3-container w3-white w3-mobile">
        <form action="{{route('user.store')}}" method="post">
            ...
            <div class="error w3-red">{{ $message }}</div>
            @enderror
            <input class="w3-input w3-border w3-round-large w3-mobile" type="password" placeholder="Enter Confirm Password" name="password_confirmation" style="width: 400px;" value="{{$user->password_confirmation}}" /><br />
            @error('articles')
            <div class="error w3-red">{{ $message }}</div>
            @enderror
            <select id="articles" name="articles" style="width: 400px;" class="w3-input w3-mobile w3-border w3-round-large">
            ...
            <input type="submit" class="w3-mobile w3-button w3-cyan w3-text-white w3-block" value="Create Account">
            </input>
            {{ csrf_field() }}
